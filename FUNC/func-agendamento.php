...
<div id="titulos">
  <h2>BLACKBEARD - Agendamentos Feitos</h2>
</div>
    ...
    <!--CRIA UMA TABELA COM AS INFORMAÇÕES-->
        <table class="table table-striped table-dark">
            ...
            <tbody>
                ...
                $sql = $conn->prepare("SELECT * FROM tb_usuarios WHERE us_tipo = 'func'");            
                $sql->execute();
                ...
                while ($test = $sql->fetch(PDO::FETCH_ASSOC)) {
                    ...
                    if ( $adm_log === 'func') {
                      ...
                      $sql = $conn->prepare("SELECT tb_agendamentos.ag_cort_barba, tb_agendamentos.ag_cort_cabelo, tb_corte_barba.tp_barba_nome, tb_corte_cabelo.tp_cabelo_nome, tb_agendamentos.log_cod_ag, tb_usuarios.us_nome, ag_cod, ag_data, ag_hora, ag_tp_pag, ag_obs FROM tb_agendamentos INNER JOIN tb_corte_barba ON ag_cort_barba = tp_barba_cod INNER JOIN tb_corte_cabelo ON ag_cort_cabelo = tp_cabelo_cod INNER JOIN tb_usuarios ON log_cod_ag = us_cpf WHERE func_cod_ag = $cpf ORDER BY ag_cod DESC");
                      $sql->execute();
