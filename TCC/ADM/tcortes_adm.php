...
<div id="titulos">
  <h2>BLACKBEARD - Cortes Cadastrados</h2>
</div>
...
<form action="../PHP/test_log.php" method="POST">
        <table class="table table-striped table-dark">
            <thead>
                <tr>
                    <th scope="col">Barba COD</th>
            ...
            <tbody>
                ...
                while ($test = $sql->fetch(PDO::FETCH_ASSOC)) {
                    ...
                    if ( $adm_log === 'adm') {
                      ...
                      $sql = $conn->prepare("SELECT * FROM tb_corte_barba");            
                      $sql->execute();
                ...
                while($user_data=$sql->fetch(PDO::FETCH_ASSOC)) {
                  ?>
                         <tr>
                             <!--MOSTRA COD DA HORA-->
                          <td><?php echo $user_data['tp_barba_cod']; ?></td>
                          ...
                          <a class='btn btn-sm btn-danger' name='del' title='Deletar' href=" <?php echo "delete/delete_barba.php?tp_barba_cod={$user_data['tp_barba_cod']}";?>">
            ...
            <?php                
                      ...
                      //PEGA TODOS OS DADOS DA TABELA DE CLIENTES
                      $sql2 = $conn->prepare("SELECT * FROM tb_corte_cabelo");            
                      $sql2->execute();
                     
                
                //ASSOCIA OS DADOS DO BANCO A VARIAVEL
                while($teste=$sql2->fetch(PDO::FETCH_ASSOC)) {
                  ?>
                         <tr>
                             
                            <!--MOSTRA COD DA HORA-->
                          <td><?php echo $teste['tp_cabelo_cod']; ?></td>
                            
                             <!--MOSTRA O NOME DO FUNCIONARIO-->
                          <td><?php echo $teste['tp_cabelo_nome']; ?></td>
                          
                          <td>
                              <!--CRIA UM BOTÃO QUE PERMITE DELETAR OS DADOS-->
                          <a class='btn btn-sm btn-danger' name='del' title='Deletar' href=" <?php echo "delete/delete_cabelo.php?tp_cabelo_cod={$teste['tp_cabelo_cod']}";?>">
                          <svg xmlns='http://www.w3.org/2000/svg' width='16' height='16' fill='currentColor' class='bi bi-trash-fill' viewBox='0 0 16 16'>
                          <path d='M2.5 1a1 1 0 0 0-1 1v1a1 1 0 0 0 1 1H3v9a2 2 0 0 0 2 2h6a2 2 0 0 0 2-2V4h.5a1 1 0 0 0 1-1V2a1 1 0 0 0-1-1H10a1 1 0 0 0-1-1H7a1 1 0 0 0-1 1H2.5zm3 4a.5.5 0 0 1 .5.5v7a.5.5 0 0 1-1 0v-7a.5.5 0 0 1 .5-.5zM8 5a.5.5 0 0 1 .5.5v7a.5.5 0 0 1-1 0v-7A.5.5 0 0 1 8 5zm3 .5v7a.5.5 0 0 1-1 0v-7a.5.5 0 0 1 1 0z'/>
                          </svg>
                          </a>
