...
<!--CRIA UMA TABELA COM AS INFORMAÇÕES-->
    ...
    <table class="table table-striped table-dark">
        <thead>
            <tr>
                <th scope="col">CODIGO DO AGENDAMENTO</th>
...
<option value="all">FUNCIONÁRIOS</option>
  <?php 
//CONEXÃO COM O BANCO
...
<div><!--EXECUTA AS VERIFICAÇÕES-->
    <button type="submit" class="btn btn-primary" name="pesquisa">Pesquisar</button>
  </div>
        ...
        <tbody>
            ...
            while ($test = $sql->fetch(PDO::FETCH_ASSOC)) {
                ...
                if ( $adm_log === 'adm') {
                    ...
                    //PEGA TODOS OS DADOS DA TABELA DE AGENDAMENTOS E MOSTRA EM ORDE DECRESCENTE
                  $sql = $conn->prepare("SELECT tb_agendamentos.ag_cort_barba, tb_agendamentos.ag_cort_cabelo, 
                  tb_corte_barba.tp_barba_nome, tb_corte_cabelo.tp_cabelo_nome, tb_agendamentos.log_cod_ag,
                  tb_agendamentos.func_cod_ag, tb_usuarios.us_nome, tb_usuarios.us_tipo, ag_cod, ag_data, ag_hora, ag_tp_pag, ag_obs 
                  FROM tb_agendamentos 
                  INNER JOIN tb_corte_barba ON ag_cort_barba = tp_barba_cod 
                  INNER JOIN tb_corte_cabelo ON ag_cort_cabelo = tp_cabelo_cod 
                  INNER JOIN tb_usuarios ON log_cod_ag = us_cpf WHERE func_cod_ag = $func
                  
                  ORDER BY ag_cod DESC
                  ");
            ...
            while($user_data=$sql->fetch(PDO::FETCH_ASSOC)) {
              ...
              ?>
                      ...
                      <td colspan="8">
                          ...
                          <a class='btn btn-sm btn-danger' name='del' title='Deletar' href=" <?php echo "delete/delete_ag.php?ag_cod={$user_data['ag_cod']}";?>">
