               ...
               while ($test = $sql->fetch(PDO::FETCH_ASSOC)) {
                   ...
                   if ( $adm_log === 'adm') {
                    ...
                    if($dados_cli != null){
                      ...
                      $sql = $conn->prepare("SELECT case us_sexo when 'm' then 'Masculino' else 'Feminino' end As compl_sexo, case us_situacao when 'blk' then 'Bloqueado' else 'Ativo' end As compl_sit, us_nome, us_tell, us_dat_nasc, us_cpf, us_situacao FROM tb_usuarios where us_nome like '%$dados_cli%' and us_tipo = 'cli'");            
                      $sql->execute();
                    
                   }else{
                      $sql = $conn->prepare("SELECT case us_sexo when 'm' then 'Masculino' else 'Feminino' end As compl_sexo, case us_situacao when 'blk' then 'Bloqueado' else 'Ativo' end As compl_sit, us_nome, us_tell, us_dat_nasc, us_cpf, us_situacao FROM tb_usuarios where us_tipo = 'cli'");            
                      $sql->execute();
                   }
