...
<form action="" method="POST">
  ...
  <div class="form-group container w-70 p-2">
    <input type="text" class="form-control" id="dados_func" name="dados_func" placeholder="Digite o nome do funcionário">
    <button class="btn btn-primary" type="submit" name="pesquisar">PESQUISAR</button>
  </div>
</fomr>
                </tr>
                <tr>
                    <th scope="col">Hora COD</th>
            ...
            <tbody>
                ...
                while ($test = $sql->fetch(PDO::FETCH_ASSOC)) {
                    ...
                    if(isset($_POST['pesquisar'])){
                     $dados_func = $_POST['dados_func'];
                     
                    if($dados_func != null){
                    
                      $sql = $conn->prepare("SELECT tb_horarios.hr_func_cod, tb_usuarios.us_nome, hr_cod, hr_hora, hr_disponivel FROM tb_usuarios INNER JOIN tb_horarios ON us_cpf = hr_func_cod WHERE us_nome like '%$dados_func%' ORDER BY us_nome");            
                      $sql->execute();
                    
                   }else{
                      $sql = $conn->prepare("SELECT tb_horarios.hr_func_cod, tb_usuarios.us_nome, hr_cod, hr_hora, hr_disponivel FROM tb_usuarios INNER JOIN tb_horarios ON us_cpf = hr_func_cod ORDER BY us_nome, hr_hora");            
                      $sql->execute();
                   }
                   }
                    }
                ...
                while($user_data=$sql->fetch(PDO::FETCH_ASSOC)) {
                  ?>
                         ...
                         <tr>
                             <!--MOSTRA COD DA HORA-->
                          <td><?php echo $user_data['hr_cod']; ?></td>
                            
                             <!--MOSTRA O NOME DO FUNCIONARIO-->
                          <td><?php echo $user_data['us_nome']; ?></td>
                          
                            <!--MOSTRA A HORA DO FUNCIONARIO-->
                          <td><?php echo date("h:i", strtotime($user_data['hr_hora'])); ?></td>
                          
                             <!--MOSTRA SE HORA ESTA OU NAO DISPONIVEL-->
                          <td><?php echo $user_data['hr_disponivel']; ?></td>
                          
                          <td>
                          <a class='btn btn-sm btn-primary' name='atualizar' title='atualizar' href=" <?php echo "atualizar_horarios.php?hr_cod={$user_data['hr_cod']}";?>">
                          ...
                          <a class='btn btn-sm btn-danger' name='del' title='Deletar' href=" <?php echo "delete/delete_horarios.php?hr_cod={$user_data['hr_cod']}";?>">
