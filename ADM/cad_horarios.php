...
header("location: ../index.html"); 
    ...
    if(isset($_POST['submit'])){
        ...
        //FAZ VERIFICAÇOES DE VALIDAÇÃO DOS DADOS
        if($funcionario = null){
          $erro_cad = "Obrigatorio escolha do funcionário";
          
        }elseif($horario = null){
            $erro_cad = "Obrigatorio inserir um horário";
            ...
            $sql = "INSERT INTO tb_horarios (hr_func_cod, hr_hora, hr_disponivel) VALUES ($funcionario, '$horario', 'v')";
...
<div id="titulos">
<h2>BLACKBEARD - Cadastro horários</h2><br>
</div>
<div class="cadastro container w-50" >
   
<label class="form-label" for="senha" > <h5>Escolha o Funcionário:</h5> </label><span class="error"> *<?=$erro_cad ?></span>
...
<option value="0">Funcionários</option>
  ...
  $stmt = $conn->prepare("SELECT * FROM tb_usuarios WHERE us_situacao = 'atv' AND us_tipo = 'func'");
  $stmt->execute();
  ...
  if($stmt->rowCount() > 0 ){
    ...
    while($dados = $stmt->fetch(PDO::FETCH_ASSOC)){
      ...
      <option value= "<?php print_r($dados['us_cpf']); ?>">
      <?php echo $dados['us_nome'];?></option>
  ...
  <div>
    <label class="form-label" for="horario"><h5>Horario Disponivel:</h5> 
