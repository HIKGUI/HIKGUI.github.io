...
<form action="" method="POST">
  ...
  <div class="form-group container w-70 p-2">
    <input type="text" class="form-control" id="dados_func" name="dados_func" placeholder="Digite o nome do cliente">
            ...
            <tbody>
                ...
                while ($test = $sql->fetch(PDO::FETCH_ASSOC)) {
                    ...
                    if($dados_func != null){
                      ...
                      $sql = $conn->prepare("SELECT case us_sexo when 'm' then 'Masculino' else 'Feminino' end As compl_sexo, case us_situacao when 'blk' then 'Bloqueado' else 'Ativo' end As compl_sit, us_nome, us_tell, us_dat_nasc, us_cpf, us_situacao FROM tb_usuarios where us_nome like '%$dados_func%' and us_tipo = 'func' or us_tipo = 'atd'");            
                      $sql->execute();
                    
                   }else{
                     //PEGA TODOS OS DADOS DA TABELA DE CLIENTES
                      $sql = $conn->prepare("SELECT case us_sexo when 'm' then 'Masculino' else 'Feminino' end As compl_sexo, case us_situacao when 'blk' then 'Bloqueado' else 'Ativo' end As compl_sit, us_nome, us_tell, us_dat_nasc, us_cpf, us_situacao FROM tb_usuarios where us_tipo = 'func' or us_tipo = 'atd'");
                      $sql->execute();
                   }
                ...
                while($user_data=$sql->fetch(PDO::FETCH_ASSOC)) {
                  ?>
                         <tr>
                          ...
                          <!--CRIA UM BOTÃO QUE TORNA FUNCIOÁRIO EM ADM-->
                        <a class='btn btn-sm btn-secondary' title='Tornar ADM' href=" <?php echo "saveUp_adm.php?cpf={$user_data['us_cpf']}";?>">
